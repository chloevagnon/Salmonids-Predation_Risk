...
# 1) "Param_Depth_Matching" : find the best association of parameters (meanlog and sdlog) for fittig the log normal density curve
...
#                                
# 2) "Depth_Matching" : find the spatial vulnerability of a prey depending on its depth according to the density curve previously fitted.
                       

# FUNCTION 1 : Param_Depth_Matching
#
# Inputs :
...
# - MonthNum=the numeric value corresponding to the month number (e.g. January = 1, February=2)

# Outputs : 
# A list of length equal to the number of months countaining : 
...
Param_Depth_Matching<-function(DepthAll,DepthMod,MEAN,SD,MonthNum){
...
#                - a list resulting from the function "Param_Depth_Matching_SD"
...
Depth_Matching<-function(Param,DepthPrey,Month,DepthAll){
  
  Depth_Match<-data.frame(Month=rep(Month,each=length(DepthPrey)),
                                    Depth=rep(DepthPrey),Vul=NA)
  
  if(class(Param)=="list"){
    for(i in as.character(Depth_Match$Month)){
      for(j in Depth_Match$Depth){
        Depth_Match$Vul[Depth_Match$Month==i & Depth_Match$Depth==j]<-Param[["Density"]][[i]]$Prob[which(Param[["Density"]][[i]]$depth==j)]
        }
      }
  }
